...
## Simulated datasets
This folder contains the genome assemblies of 7 S. cerevisiae and 5 S. paradoxus strains modified to contain synthetic telomeres of known length. The subfolders "Aneuploidy", "Ploidy" and "Telomeric mutations" contain the genome assembly of the reference strain S288c modified to contain aneuploidies, changes in ploidy and mutations in telomeric sequences, respectively. The original genome assemblies were part of another study (https://yjx1217.github.io/Yeast_PacBio_2016/welcome/). The list of synthetic telomeres which have been used for replacement is contained in the file "Synthetic_telomeric_repeats.fa".
...
## Other scripts
This folder contains the script used to annotate Interstitial Telomeric Sequences (ITS) in the Yeast Population Reference Panel (YPRP). Usage:

`$ perl find_motif_in_genome.pl -i $GENOME.fasta -m motif.txt -p $prefix`
...
If other studies would like to use the large amount of telomere length data supplied in our paper, a sound way to compare results would be to include the representative strains of the teloblot shown in figure 1b in the new sequencing batch. The TL estimations of these shared strains should be compared between the sequencing batches and the teloblot values, in order to re-calculate correction factors for both batches. 
